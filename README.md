...
<title>AI 穿搭生態系統｜拍照建立衣櫃，AI 幫你配每日穿搭</title>
<meta name="description" content="拍照把衣服收進數位衣櫃，AI 依場合與天氣自動幫你配好整套穿搭，並記錄與分析你的穿衣習慣。靜宜大學資料科學系專題。">
...
/* ---------- HEADER ---------- */
header{position:sticky;top:0;z-index:100;background:rgba(10,10,11,.72);backdrop-filter:blur(18px);border-bottom:1px solid var(--line)}
.topbar{max-width:var(--maxw);margin:0 auto;display:flex;align-items:center;justify-content:space-between;padding:14px 28px;gap:20px}
.brand{display:flex;align-items:center;gap:14px;cursor:pointer;user-select:none}
.brand .logo{width:38px;height:38px;border:1.5px solid var(--text);display:grid;place-items:center;font-family:"Anton";font-size:20px;transition:.35s}
...
.navlink{background:none;border:none;color:var(--muted);cursor:pointer;font-family:"Noto Sans TC";font-size:13px;font-weight:500;padding:9px 13px;letter-spacing:.5px;position:relative;transition:.25s;white-space:nowrap}
.navlink:hover{color:var(--text)}
.navlink.active{color:var(--text)}
.navlink.active::after{content:"";position:absolute;left:13px;right:13px;bottom:2px;height:2px;background:var(--accent)}
...
.hero h1{font-family:"Anton";font-weight:400;font-size:clamp(48px,11vw,150px);line-height:.86;letter-spacing:-1px;text-transform:uppercase}
...
.hero p.lede{max-width:620px;color:var(--text);margin-top:18px;font-size:17px;line-height:1.7}
.hero p.lede b{color:var(--accent);font-weight:700}
.hero-cta{display:flex;gap:14px;margin-top:30px;flex-wrap:wrap}
.btn{font-family:"Noto Sans TC";font-weight:700;font-size:14px;letter-spacing:1px;padding:15px 30px;border:1.5px solid var(--text);background:none;color:var(--text);cursor:pointer;transition:.3s;display:inline-flex;align-items:center;gap:10px}
...
/* 三步驟流程 */
.steps{display:flex;align-items:stretch;gap:14px;margin-top:42px;flex-wrap:wrap}
.step-card{flex:1;min-width:210px;display:flex;gap:16px;align-items:flex-start;background:var(--surface);border:1px solid var(--line);padding:22px 24px;transition:.3s}
.step-card:hover{border-color:var(--accent)}
.step-card .sn{font-family:"Anton";font-size:30px;color:var(--accent);line-height:1;flex-shrink:0;width:32px}
.step-card b{font-size:16px;font-weight:700;display:block;margin-bottom:5px}
.step-card p{font-size:13px;color:var(--muted);margin:0}
.step-arrow{display:flex;align-items:center;color:var(--dim);font-family:"Space Mono";font-size:22px}

.stat-row{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:var(--line);border:1px solid var(--line);margin-top:60px}
...
.field .opt{color:var(--dim);font-size:10px}
.field input,.field select,.field textarea{width:100%;background:var(--surface);border:1px solid var(--line);color:var(--text);padding:13px 15px;font-family:"Noto Sans TC";font-size:14px;transition:.25s}
...
<header>
  <div class="topbar">
    ...
    <nav id="nav">
      ...
      <button class="navlink" data-v="recommend" onclick="go('recommend')">AI 幫我配<span class="en">AI MATCH</span></button>
      <button class="navlink" data-v="records" onclick="go('records')">穿搭紀錄<span class="en">LOOKS</span></button>
      <button class="navlink" data-v="history" onclick="go('history')">操作紀錄<span class="en">HISTORY</span></button>
...
<main>
  ...
  <section class="view active" id="view-home">
    <div class="hero">
      ...
      <p class="lede">拍照把衣服收進手機裡的數位衣櫃,<b>AI 會依「場合 + 天氣」幫你配好一整套</b>,還能記錄每天穿搭、用數據看懂自己的穿衣習慣。</p>
      <div class="hero-cta">
        <button class="btn fill" onclick="go('add')">① 上傳第一件衣服 <span class="ar">→</span></button>
        <button class="btn" onclick="go('recommend')">✦ AI 穿搭</button>
        <button class="btn" onclick="loadSample()">先看範例衣櫃</button>
      </div>

      <div class="steps">
        <div class="step-card"><span class="sn">1</span><div><b>上傳衣服</b><p>拍照或選圖,把每件衣服存進數位衣櫃。</p></div></div>
        <div class="step-arrow">→</div>
        <div class="step-card"><span class="sn">2</span><div><b>AI 幫你配</b><p>選場合與天氣,系統自動配出一整套穿搭。</p></div></div>
        <div class="step-arrow">→</div>
        <div class="step-card"><span class="sn">3</span><div><b>記錄與分析</b><p>採用的穿搭會被記錄,並轉成穿衣習慣數據。</p></div></div>
      </div>
    </div>

    <div class="stat-row">
      <div class="stat"><div class="num" id="h-total">0</div><div class="lbl">衣物總數<span class="en">TOTAL ITEMS</span></div></div>
      <div class="stat"><div class="num" id="h-looks">0</div><div class="lbl">穿搭紀錄<span class="en">LOGGED LOOKS</span></div></div>
      <div class="stat"><div class="num" id="h-cats">0</div><div class="lbl">衣物分類<span class="en">CATEGORIES</span></div></div>
      <div class="stat"><div class="num" id="h-wears">0</div><div class="lbl">總穿著次數<span class="en">TOTAL WEARS</span></div></div>
    </div>

    <div class="feature-grid">
      <div class="fcard" onclick="go('add')"><span class="idx">01</span><div class="ic">＋</div><h3>新增衣物</h3><p>上傳照片、選擇分類與天氣,建立你的數位衣物資料庫。</p><span class="go">前往 ADD CLOTHES →</span></div>
      <div class="fcard" onclick="go('wardrobe')"><span class="idx">02</span><div class="ic">▦</div><h3>我的衣櫃</h3><p>以商品牆方式瀏覽全部衣物,點擊查看詳細資訊。</p><span class="go">前往 WARDROBE →</span></div>
      <div class="fcard" onclick="go('recommend')"><span class="idx">03</span><div class="ic">✦</div><h3>AI 幫我配</h3><p>依場合與天氣,演算法自動組合一套協調的穿搭。</p><span class="go">前往 AI MATCH →</span></div>
      ...
      <div class="fcard" onclick="go('analytics')"><span class="idx">05</span><div class="ic">◔</div><h3>數據分析</h3><p>分類比例、天氣分布、穿著頻率,一目了然。</p><span class="go">前往 ANALYTICS →</span></div>
  ...
  <section class="view" id="view-add">
    <div class="section-head"><div><div class="eyebrow">STEP 01 / INPUT</div><h2>ADD CLOTHES</h2><div class="zh">新增衣物 — 只要「照片 + 名稱」就能建立一件</div></div></div>
    <div class="form-wrap">
      <div class="form-pane">
        <div class="field"><label>衣物名稱 <span class="req">＊必填</span></label><input id="f-name" placeholder="例：白色基本款 T 恤"></div>
        <div class="field"><div class="row">
          <div><label>分類</label><select id="f-cat"></select></div>
          <div><label>適合天氣</label><select id="f-weather"></select></div>
        </div></div>
        <div class="field"><label>衣物照片 <span class="opt">（選填，建議上傳）</span></label>
        ...
        <div class="field"><label>備註 <span class="opt">（選填）</span></label><textarea id="f-note" placeholder="材質、購入時間、搭配建議…"></textarea></div>
  ...
  <section class="view" id="view-recommend">
    <div class="section-head"><div><div class="eyebrow">AI STYLING ENGINE</div><h2>AI MATCH</h2><div class="zh">AI 幫我配 — 選好場合與天氣,自動配出一整套</div></div></div>
    <div class="rec-controls">
      <div class="field"><label>場合</label><select id="r-occ"><option>日常休閒</option><option>上班通勤</option><option>正式場合</option><option>運動</option><option>約會</option></select></div>
      <div class="field"><label>天氣氣候</label><select id="r-weather"></select></div>
    ...
    <div class="rec-meta" id="rec-meta" style="display:none">
      <div class="m"><div class="k">搭配分數 SCORE</div><div class="v" id="rec-score">—</div><div class="score-bar"><i id="rec-scorebar"></i></div></div>
      <div class="m"><div class="k">適合場合 OCCASION</div><div class="v" id="rec-occ">—</div></div>
      <div class="m"><div class="k">天氣 WEATHER</div><div class="v" id="rec-weather">—</div></div>
  ...
  <section class="view" id="view-history">
    <div class="section-head"><div><div class="eyebrow">ACTIVITY LOG</div><h2>HISTORY</h2><div class="zh">操作紀錄 — 系統操作時間軸</div></div>
  ...
  <section class="view" id="view-analytics">
    ...
    <div class="an-grid">
      ...
      <div class="panel full"><h3>最常穿著 TOP 5 <span class="en">MOST WORN</span></h3><div class="sub">穿著次數最高的單品</div><div id="worn-bars"></div></div>
    </div>
  </section>
  ...
  <section class="view" id="view-about">
    ...
    <div class="about-grid">
      <div class="about-pane">
        ...
        <p>衣物資料與照片管理、依場合與天氣的 AI 穿搭推薦引擎、穿搭紀錄日誌、操作歷史軌跡,以及涵蓋分類 / 天氣 / 穿著頻率的數據分析儀表板。</p>
      ...
      <div class="about-pane">
        <h3>系統運作流程</h3>
        <div class="flow">
          <div class="step"><div class="no">01</div><div class="sb"><h4>建檔 INPUT</h4><p>使用者上傳照片並輸入分類、天氣等結構化屬性。</p></div></div>
          <div class="step"><div class="no">02</div><div class="sb"><h4>儲存 STORE</h4><p>照片經裁切壓縮後與資料一起寫入本機儲存,形成個人衣物資料庫。</p></div></div>
          <div class="step"><div class="no">03</div><div class="sb"><h4>運算 MATCH</h4><p>推薦引擎依場合 / 天氣協調規則,組合候選穿搭並評分。</p></div></div>
...
/* 範例衣櫃（使用者按「先看範例衣櫃」才會載入，非自動） */
const SAMPLE=[
  {name:"白色基本款 T 恤",cat:"top",weather:"炎熱",wear:6},
  {name:"灰色連帽帽 T",cat:"top",weather:"涼爽",wear:3},
  {name:"條紋長袖上衣",cat:"top",weather:"溫暖",wear:2},
  {name:"水洗直筒牛仔褲",cat:"bottom",weather:"通用",wear:8},
  {name:"卡其工裝褲",cat:"bottom",weather:"涼爽",wear:2},
  {name:"黑色寬版西裝外套",cat:"outer",weather:"涼爽",wear:4},
  {name:"深藍羽絨外套",cat:"outer",weather:"寒冷",wear:5},
  {name:"白色帆布鞋",cat:"shoes",weather:"通用",wear:9},
  {name:"黑色皮革短靴",cat:"shoes",weather:"寒冷",wear:2},
  {name:"棒球帽",cat:"acc",weather:"通用",wear:3},
  {name:"碎花連身洋裝",cat:"set",weather:"溫暖",wear:4},
  {name:"針織連身套裝",cat:"set",weather:"涼爽",wear:1}
];
...
function load(){
  ...
  state={clothes:[],records:[],history:[]};   /* 預設空白，不載入任何資料 */
  save();
}
...
function loadSample(){
  const run=()=>{
    const items=SAMPLE.map(s=>({id:uid(),name:s.name,cat:s.cat,weather:s.weather,note:"",img:null,wear:s.wear,added:Date.now()}));
    state.clothes=items;
    const find=k=>items.find(c=>c.cat===k);
    const r1=[find("top"),find("bottom"),find("shoes")].filter(Boolean);
    const r2=[find("set"),find("shoes")].filter(Boolean);
    state.records=[
      {id:uid(),date:Date.now()-86400000,occ:"日常休閒",weather:"通用",itemIds:r1.map(c=>c.id),items:r1.map(c=>({name:c.name,cat:c.cat,img:null}))},
      {id:uid(),date:Date.now()-2*86400000,occ:"約會",weather:"溫暖",itemIds:r2.map(c=>c.id),items:r2.map(c=>({name:c.name,cat:c.cat,img:null}))}
    ];
    state.history=[{id:uid(),time:Date.now(),type:"sys",text:"已載入範例衣櫃 — "+items.length+" 件單品"}];
    filter="all";save();renderHome();toast("已載入範例衣櫃 ✓");go("wardrobe");
  };
  if(state.clothes.length>0||state.records.length>0){
    showConfirm({title:"載入範例衣櫃?",msg:"這會以範例資料<b>取代</b>目前的衣櫃與紀錄,方便你預覽成品的樣子。<br>之後可用「清空衣櫃」還原成空白。",confirmLabel:"載入範例"},run);
  }else run();
}
...
function readImageFile(file,cb){
  ...
  reader.onload=e=>{
    ...
    img.onload=()=>{
      ...
      const ar=img.width/img.height;let sw,sh,sx,sy;
...
function animateCount(el,target){
  ...
  function step(now){const p=Math.min((now-start)/dur,1);el.textContent=Math.round(from+(target-from)*(1-Math.pow(1-p,3)));if(p<1)requestAnimationFrame(step);}
  requestAnimationFrame(step);
}
function renderHome(){
  animateCount(document.getElementById("h-total"),state.clothes.length);
  animateCount(document.getElementById("h-looks"),state.records.length);
  animateCount(document.getElementById("h-cats"),new Set(state.clothes.map(c=>c.cat)).size);
  animateCount(document.getElementById("h-wears"),state.clothes.reduce((a,c)=>a+(c.wear||0),0));
}

/* ===================== ADD ===================== */
function initForm(){
  document.getElementById("f-cat").innerHTML=CATEGORIES.map(c=>`<option value="${c.key}">${c.zh}</option>`).join("");
  document.getElementById("f-weather").innerHTML=WEATHERS.map(w=>`<option>${w}</option>`).join("");
  ["f-name","f-cat","f-weather"].forEach(id=>document.getElementById(id).addEventListener("input",updatePreview));
  document.getElementById("f-file").addEventListener("change",e=>{
    const file=e.target.files[0];if(!file)return;
    readImageFile(file,data=>{if(!data){toast("圖片讀取失敗，請換一張","!");return;}selImg=data;showThumb();updatePreview();});
  });
  updatePreview();
}
function showThumb(){
  const thumb=document.getElementById("f-thumb"),drop=document.getElementById("f-drop");
  if(selImg){
    document.getElementById("f-thumb-img").src=selImg;thumb.style.display="block";
...
function clearPhoto(e){if(e)e.stopPropagation();selImg=null;document.getElementById("f-file").value="";showThumb();updatePreview();}
function updatePreview(){
  ...
  document.getElementById("p-cat").textContent=c.zh;
  ...
  document.getElementById("p-tags").innerHTML=`<span class="tag">${c.zh}</span><span class="tag">${weather}</span>`;
}
function addClothes(){
  ...
  save();toast("已加入衣櫃：「"+name+"」");resetForm();go("wardrobe");
}
function resetForm(){
...
function renderWardrobe(){
  ...
  f.innerHTML=all.map(c=>`<button class="chip ${filter===c.key?'active':''}" onclick="setFilter('${c.key}')">${c.zh} <span style="opacity:.5">${countCat(c.key)}</span></button>`).join("");
  const grid=document.getElementById("wardrobe-grid");
  const items=state.clothes.filter(c=>filter==="all"||c.cat===filter);
  if(items.length===0){grid.innerHTML=`<div class="empty"><div class="big">∅</div>${state.clothes.length===0?'你的衣櫃還是空的，先新增第一件衣物，或回首頁點「先看範例衣櫃」。':'這個分類還沒有衣物。'}<br><button class="btn fill" style="margin-top:18px" onclick="go('add')">＋ 新增衣物</button></div>`;return}
  grid.innerHTML=items.map((c,i)=>{
    ...
    return `<div class="product" onclick="openModal('${c.id}')">
      ...
      <div class="info">
        ...
        <div class="cat">${cat.zh} · ${c.weather||'通用'}</div>
      </div>
...
function openModal(id){
  ...
  document.getElementById("m-cat").textContent=cat.zh;
  ...
  document.getElementById("m-details").innerHTML=`
    ...
    <div class="r"><span class="k">適合天氣 WEATHER</span><span>${c.weather||'通用'}</span></div>
...
function wearItem(id){
  ...
  save();toast("已記錄穿著：「"+c.name+"」");renderWardrobe();openModal(id);
}
function delItem(id){
  const c=state.clothes.find(x=>x.id===id);if(!c)return;
  showConfirm({title:"刪除這件衣物?",msg:`即將從衣櫃刪除「<b>${c.name}</b>」。<br>此動作無法復原。`,confirmLabel:"確認刪除"},()=>{
...
function renderOutfitEmpty(){
  document.getElementById("outfit").innerHTML=`<div class="slot miss" style="grid-column:1/-1;min-height:200px">${state.clothes.length===0?'衣櫃還是空的，請先到「新增衣物」上傳幾件單品 →':'選好場合與天氣，點「生成穿搭」,讓 AI 幫你配一整套 ✦'}</div>`;
  document.getElementById("rec-meta").style.display="none";
}
function pick(cat,weather){
...
function recommend(){
  if(state.clothes.length===0){renderOutfitEmpty();toast("衣櫃是空的，先去新增衣物吧","!");return}
  const occ=document.getElementById("r-occ").value;
  const weather=document.getElementById("r-weather").value;
  const hasSet=state.clothes.some(c=>c.cat==="set");
  const hasTB=state.clothes.some(c=>c.cat==="top"||c.cat==="bottom");
  /* 有「整套」時，部分機率用整套取代上衣+下身；若完全沒有上衣下身則一定用整套 */
  const useSet=hasSet&&(!hasTB||Math.random()<0.5);
  let plan=useSet
    ?[["set","整套"],["outer","外套"],["shoes","鞋子"]]
    :[["outer","外套"],["top","上衣"],["bottom","下身"],["shoes","鞋子"]];
  if(weather==="炎熱")plan=plan.filter(p=>p[0]!=="outer");  /* 炎熱不搭外套 */
  const picks={};
  plan.forEach(([k])=>picks[k]=pick(k,weather));
  const hasMain=picks.set||picks.top||picks.bottom;
  if(!hasMain){
    document.getElementById("outfit").innerHTML=`<div class="slot miss" style="grid-column:1/-1;min-height:200px">衣櫃裡還缺少「上衣 / 下身 / 整套」這類主要單品,<br>先補幾件就能幫你配整套了。<br><button class="btn fill" style="margin-top:16px" onclick="go('add')">＋ 去新增衣物</button></div>`;
    document.getElementById("rec-meta").style.display="none";return;
  }
  currentRec={occ,weather,picks};
  document.getElementById("outfit").innerHTML=plan.map(([k,zh])=>{
    const c=picks[k],cat=catMap[k];
    if(!c)return `<div class="slot miss"><div><span class="slot-lbl">${zh}</span><br>缺少${zh}<br><small style="color:var(--dim)">可至「新增衣物」補齊</small></div></div>`;
    return `<div class="slot"><span class="slot-lbl">${zh}</span>
      <div class="si" style="background:var(--surface-2)">${fillVisual(c,cat.emoji)}</div>
      <div class="sm"><div class="n">${c.name}</div><div class="c">${c.weather||'通用'}</div></div></div>`;
  }).join("");
  const chosen=Object.values(picks).filter(Boolean);
  let score=70+chosen.length*6;
  ...
  document.getElementById("rec-occ").textContent=occ;
  ...
  logHistory("rec","AI 生成穿搭推薦 ("+occ+" / "+weather+")");
}
function wearThis(){
  if(!currentRec)return;
  const chosen=Object.values(currentRec.picks).filter(Boolean);
  chosen.forEach(c=>c.wear=(c.wear||0)+1);
  state.records.unshift({id:uid(),date:Date.now(),occ:currentRec.occ,weather:currentRec.weather,
...
function renderRecords(){
  ...
  if(state.records.length===0){el.innerHTML=`<div class="empty" style="border:1px solid var(--line)"><div class="big">◷</div>還沒有穿搭紀錄。<br>到「AI 幫我配」生成並採用一套吧!<br><button class="btn fill" style="margin-top:18px" onclick="go('recommend')">前往 AI 推薦 →</button></div>`;return}
  el.innerHTML=state.records.map(r=>{
    ...
    return `<div class="record-card">
      <div class="rl"><div class="occ">${r.occ}</div><div class="dt">${new Date(r.date).toLocaleString('zh-TW')} · ${r.weather||'不限'}</div></div>
...
function delRecord(id){
  ...
  showConfirm({
    ...
    checkboxDefault:true,confirmLabel:"確認刪除"
  },(revert)=>{
    if(revert&&r.itemIds){r.itemIds.forEach(iid=>{const c=state.clothes.find(x=>x.id===iid);if(c&&c.wear>0)c.wear--;});}
...
function renderHistory(){
  ...
  if(state.history.length===0){el.innerHTML=`<div style="color:var(--muted)">尚無操作紀錄。開始新增衣物或生成穿搭後，這裡會自動記錄。</div>`;return}
...
function clearHistory(){
  if(state.history.length===0){toast("目前沒有操作紀錄","!");return;}
  showConfirm({title:"清空操作紀錄?",msg:`即將清除全部 <b>${state.history.length}</b> 筆操作軌跡。<br>此動作無法復原。`,confirmLabel:"確認清空"},()=>{
    state.history=[];save();renderHistory();toast("操作紀錄已清空","↻");
  });
}
function clearWardrobe(){
  ...
  showConfirm({
    ...
    checkboxDefault:true,confirmLabel:"全部清空"
  },(alsoRecords)=>{
    state.clothes=[];if(alsoRecords)state.records=[];
...
function renderAnalytics(){
  ...
  const grays=["#f4f4f4","#cfcfcf","#a8a8a8","#828282","#5c5c5c","#363636"];
  let acc=0,segs=[];
  catCount.forEach((c,i)=>{const frac=c.n/total;segs.push({...c,start:acc,frac,col:grays[i%grays.length]});acc+=frac});
  const C=2*Math.PI*54;
  document.getElementById("donut").innerHTML=`<svg width="150" height="150" viewBox="0 0 150 150" style="transform:rotate(-90deg)">
  ...
  document.getElementById("worn-bars").innerHTML=top.length?top.map((c,i)=>
    `<div class="bar-row"><div class="bl" style="font-size:12px">${c.name.length>8?c.name.slice(0,8)+"…":c.name}</div><div class="bt"><i class="${i===0?'acc':''}" data-w="${(c.wear||0)/maxW*100}"></i></div><div class="bv">${c.wear||0}×</div></div>`).join(""):`<div class="empty-mini">尚無穿著資料</div>`;
...
function showConfirm(opts,cb){
  ...
  const wrap=document.getElementById("cf-check-wrap"),chk=document.getElementById("cf-check");
  if(opts.checkboxLabel){wrap.style.display="flex";document.getElementById("cf-check-label").textContent=opts.checkboxLabel;chk.checked=opts.checkboxDefault!==false;}
  else{wrap.style.display="none";chk.checked=false;}
  document.getElementById("cf-ok").textContent=opts.confirmLabel||"確認刪除";
  _confirmCb=cb;document.getElementById("confirm-modal").classList.add("show");
}
function closeConfirm(){document.getElementById("confirm-modal").classList.remove("show");_confirmCb=null;}
function doConfirm(){const checked=document.getElementById("cf-check").checked;const cb=_confirmCb;closeConfirm();if(cb)cb(checked);}
